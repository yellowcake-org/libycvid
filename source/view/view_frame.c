...
yc_vid_status_t yc_vid_view_frame_tick(
    ...
    for (size_t vertical_idx = 0; vertical_idx < YC_RES_MATH_GRID_SIZE_TILES; ++vertical_idx) {
        for (size_t horizontal_idx = 0; horizontal_idx < YC_RES_MATH_GRID_SIZE_TILES; ++horizontal_idx) {
            ...
            yc_vid_view_object_t *tile_floor = &view->floor.pointers[linear_idx];

            yc_vid_status_t status = YC_VID_STATUS_OK;

            // Tiles have to have only 1 set. It doesn't change.
            if (1 != tile_floor->sets->count) { return YC_VID_STATUS_CORRUPTED; }
            yc_vid_texture_set_t *set = &tile_floor->sets[tile_floor->orientations[YC_RES_MATH_ORIENTATION_NE]];
            ...
            // Set new frame index.
            tile_floor->state.frame_idx += effective;

            // Update texture.
            yc_vid_texture_t *new = &set->textures[tile_floor->state.frame_idx];
            yc_vid_texture_t *old = tile_floor->state.texture;
            tile_floor->state.texture = new;
            ...
            if (NULL != old) {
                status = renderer->texture->set_visibility(
                        old,
